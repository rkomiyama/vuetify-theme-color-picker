<template>
  <v-list-group prepend-icon="mdi-palette-advanced" ripple>
    <template v-slot:activator>
      <v-list-item-content>
        <v-list-item-title>Themes</v-list-item-title>
      </v-list-item-content>
    </template>
    <v-list-item-group v-model="themeOption" mandatory>
      <v-list-item
      ...
      >
        <v-list-item-action>
          <v-icon :color="theme.color">{{ `mdi-circle${theme.name === "Light" ? "-outline" : ""}` }}</v-icon>
        </v-list-item-action>
        <v-list-item-content>
          <v-flex class="d-flex">
...
export default {
  ...
  methods: {
    ...
    clickSwitch(themeName) {
      ...
      this.darkOrLight =
        this.darkOrLight[0] === themeName ? [otherThemeName] : [themeName];
